...
//定义定时器超时后需要发送的消息ID
static volatile osel_signal_t ccr_sig_gprs;
static volatile osel_signal_t ccr_sig_blu;

void clk_ccrx_restart(uint32_t ticks, osel_signal_t sig)
{
    ccr_sig_gprs = sig;

	if(sig == GPRS_UART_EVENT)
	{
        TBCCR1 = TB0R + 0x147;//10MS
    	TBCCTL1 &= ~CCIFG;
    	TBCCTL1 |= CCIE;
	}
	else if(sig == BLU_UART_EVENT)
	{
        ccr_sig_blu = BLU_UART_EVENT;
        TBCCR2 = TB0R + 0x147;
    	TBCCTL2 &= ~CCIFG;
    	TBCCTL2 |= CCIE;
	}
...
static void gprs_uart_date_handler(void)
{
    TBCCTL1 &= ~CCIE;
    TBCCTL1 &= ~CCIFG;
    TBCCR1 = 0;    
    osel_event_t event;
    event.sig = ccr_sig_gprs;
    event.param = NULL;
    osel_post(NULL, &gprs_event_process, &event);
}

static void blu_uart_date_handler(void)
{
    TBCCTL2 &= ~CCIE;
    TBCCTL2 &= ~CCIFG;
    TBCCR2 = 0;
    osel_event_t event;
    event.sig = ccr_sig_blu;
    event.param = NULL;
...
{
    if(time_ms == 300000ul) // 5分钟看门狗，用于保护背景值更新错�
    {
    ...
    {
        TBCCTL0 &= ~CCIE;  //由于有周期性定时器存在，TimerB不允许停�
    }
}
...
void clk_init(uint8_t system_clock_speed)
{
    ...
    TB0CTL |= TBSSEL_1 + MC_2 + TBCLR;  // 启动TimerB定时�
    
    TBCCR4 = TB0R + 0x8000;  // 1s
    TBCCTL4 |= CCIE;
}
