...
// 设备发送温度
void coap_post_temperature(void)
{
    ...
    coap_set_mid(&msg_send, 0x5000);            // Message ID 任意

    // 第四步，加入路由 Uri-Path
    uint64_t nui = NODE_ID;
    coap_add_option(&msg_send, CON_URI_PATH, (void*)&nui, 8);
    coap_add_option(&msg_send, CON_URI_PATH, (uint8_t*)"temperature", 11);
    
    // 第五步，设置CoAP Payload，仅为测试使用，没有明显含义
    char str[] = "20.11";
    coap_set_payload(&msg_send, (uint8_t*)str, ARRAY_LEN(str));
    
    // 第六步，向SSN网关发送内容
    nwk_send(GATEWAY_NUI, msg_send.buf, msg_send.len);
}
...
PROCESS_THREAD(app_coap_client_process, ev, data)
{
    ...
    while (1)
    {
        OSEL_ETIMER_DELAY(&coap_et, 1000); 
        hal_led_toggle(GREEN);
        coap_post_temperature();
    }
...
static void send_data(uint8_t *pload,uint8_t len)
{

}

static void recv_data(uint8_t *pload,uint8_t len)
{

}

static void config_id(uint8_t *pload,uint8_t len)
{

}

static void select_id(uint8_t *pload,uint8_t len)
{
...
void app_init(void)
{
    ...
    SSN_RADIO.init();
//    at_mod_init((at_send_cb)send_data,(at_recv_cb)recv_data);
//    id_cmd_register((config_callback)config_id,(select_callback)select_id);
	nwk_init();	
	mac_init();
