...
#include "stack/custom/mac/general/mac_frame.h"
...
typedef void (*nwk2app_cb)(sbuf_t *sbuf);
nwk2app_cb nwk2app;
...
static void nwk_info_cfg(void)
{
	mac_info_t *info = mac_get();
	nwk_join_req_t gw_join;
	nwk.device_type = NODE_TYPE_GATEWAY;
	nwk.father_id = 0x0001;
	nwk.hop_num = 0;
	nwk.node_nui = info->mac_pib.mac_addr;
	
	gw_join.device_type = nwk.device_type;
	gw_join.father_id = nwk.father_id;
	gw_join.nui = nwk.node_nui;
	nwk_addr_table_add(gw_join);
}
...
static void nwk_frm_data_event(sbuf_t *sbuf, nwk_hd_addr_t *nwk_hd)
{
	...
	if (nwk.device_type == NODE_TYPE_GATEWAY)
	{
        osel_int8_t s = osel_mutex_lock(OSEL_MAX_PRIO);
	...
	{   
        ...
        pbuf->data_len = *(pbuf->head) - NWK_OFFSET_SIZE + 1;
        DBG_ASSERT(pbuf != NULL __DBG_LINE);
        DBG_ASSERT(sbuf != NULL __DBG_LINE);
	...
	else
	{
		...
		if (mac_assoc_device_short_find(route_entry->next_hop))
		{
			...
			sbuf->primargs.pbuf->data_len = *(sbuf->primargs.pbuf->head) - MAC_OFFSET_SIZE + 1;
		}
		else
		{
			DBG_LOG(DBG_LEVEL_INFO, "can't find nwk_addr");
			pbuf_free(&(sbuf->primargs.pbuf) __PLINE2);
			sbuf_free(&sbuf __SLINE2);
			return;
		}
...
static void nwk_frm_join_request_event(sbuf_t *sbuf, nwk_hd_addr_t *nwk_hd)
{
	...
	nwk_addr_table_add(nwk_join_req);
	route_entry.nui = nwk_join_req.nui;
	route_entry.nwk_addr = nwk_addr_get(&nwk_join_req);
	...
	nwk_join_resp.father_id = nwk_join_req.father_id;
	nwk_join_resp.hop_num = nwk_join_req.hop_num;
	nwk_join_resp.join_res = NET_JOIN_OK;
	nwk_join_resp.nui = route_entry.nui;
	nwk_join_resp.nwk_id = route_entry.nwk_addr;
	nwk_join_resp.reserved = 0;
	nwk_frames_join_resp_fill(pbuf, &nwk_join_resp);
	
	nwk_hd->nwk_hd_ctl.frm_ctrl = NWK_FRM_TYPE_JOIN_RESP;
	nwk_hd->nwk_hd_ctl.dst_mode = NWK_ADDR_MODE_SHORT;
	nwk_hd->nwk_hd_ctl.src_mode = NWK_ADDR_MODE_SHORT;
	nwk_hd->dst_addr = route_entry.nwk_addr;
	nwk_hd->src_addr = nwk.nwk_addr;
	nwk_hd->qos_level = QOS_LEVEL_TIME;
	nwk_frames_hd_addr_fill(pbuf, nwk_hd);
	
	sbuf->up_down_link = DOWN_LINK;
	sbuf->primargs.prim_arg.mac_prim_arg.dst_mode = MAC_MHR_ADDR_MODE_SHORT;
	sbuf->primargs.prim_arg.mac_prim_arg.dst_addr = mac_short_addr_get(route_entry.nui);
	sbuf->primargs.prim_arg.mac_prim_arg.src_mode = MAC_MHR_ADDR_MODE_SHORT;
	sbuf->primargs.prim_arg.mac_prim_arg.src_addr = mac_short_addr_get(nwk.node_nui);
...
static void nwk_frm_route_request_event(sbuf_t *sbuf, nwk_hd_addr_t *nwk_hd)
{
	...
	if (route_req_node == NULL) //*< 请求的目的NUI不在自己的路由表里
	{
		DBG_LOG(DBG_LEVEL_INFO, "can not find id\n");   
		nwk_route_resp.dst_nwk_id = 0xffff;
	}
	else                        //*< 请求的目的NUI在自己的路由表里
	{
		...
		if (!list_empty(&query_head))
		{
			...
			else
			{
				...
				sbuf_free(&sbuf __SLINE2);
                            ...
                            void nwk2app_register(void *cb)
                            {
                                nwk2app = (nwk2app_cb)cb;
                            }
