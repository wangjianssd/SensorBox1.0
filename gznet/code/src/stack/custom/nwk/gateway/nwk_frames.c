...
{
	uint8_t *datap = NULL;
	datap = pbuf->head + MAC_OFFSET_SIZE;
	datap += sizeof(nwk_hd_ctl_t);	//*< nwk head ctrl
	datap += sizeof(qos_level_t);	//*< nwk qos
	datap += sizeof(uint8_t);		//*< nwk seq

	osel_memcpy(datap, &addr, sizeof(uint16_t)); //*< 修改目的短地址
}
...
uint8_t nwk_frames_join_req_get(pbuf_t *pbuf, nwk_join_req_t *req)
{
	...
	uint8_t len = pbuf->data_p - p_data;
	pbuf->data_len -= (len + MAC_OFFSET_SIZE);
...
uint8_t nwk_frames_route_resp_fill(pbuf_t *pbuf, nwk_route_resp_t *resp)
{
	...
	osel_memcpy(pbuf->data_p, resp, sizeof(nwk_route_resp_t));
