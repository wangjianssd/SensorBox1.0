...
{
	supf_spec_t *supf = &sync_attribute->supf_cfg_arg;
...
//	uint8_t i = 0;
	...
	uint8_t beacon_cnt           = 1;
    uint8_t intra_gts_number        = supf->intra_gts_number;
	*start = (beacon_cnt+intra_gts_number)*sync_attribute->coord_cluster_index + 1;
...
void super_frame_cfg(sync_attribute_t *sync_attribute, uint8_t type)
{
    ...
    supf_cfg = &sync_attribute->supf_cfg_arg;
    ...
    slot_node_cfg(&beacon_slot, beacon_duration, 1, sync_attribute->time_slot[BEACON_SLOT],
                  &super_frame_slot, NULL, &intra_gts_slot);
    slot_node_cfg(&intra_gts_slot, gts_duration, intra_gts_number,
                  sync_attribute->time_slot[INTRA_GTS_SLOT], &super_frame_slot, NULL, NULL);
    slot_node_cfg(&super_frame_slot, 0, cluster_number, sync_attribute->time_slot[SUPER_FRAME_SLOT],
                  &intracom_slot, &beacon_slot, NULL);
    slot_node_cfg(&intracom_slot, 0, 1, sync_attribute->time_slot[INTRACOM_SLOT],
    ...
    for (int i = 0; i < inter_unit_number; i++)
    {
        slot_node_cfg(&inter_gts_slot[i], gts_duration, inter_gts_number[i],
                      sync_attribute->time_slot[INTER_GTS_SLOT], &intercom_sub_slot[i], NULL, NULL);
    }
    for (int i = 0; i < inter_unit_number; i++)
    {
        if (i == inter_unit_number - 1)
        {
            slot_node_cfg(&intercom_sub_slot[i], 0, 1, sync_attribute->time_slot[INTERCOM_SUB_SLOT],
    ...
    slot_node_cfg(&intercom_slot, 0, 1, sync_attribute->time_slot[INTERCOM_SLOT],
                  &beacon_interval, &intercom_sub_slot[0], &capcom_slot);
    slot_node_cfg(&sub_cap_slot, gts_duration, cap_repeat_cnt,
                  sync_attribute->time_slot[SUB_CAP_SLOT], &capcom_slot, NULL, NULL);
    slot_node_cfg(&capcom_slot, 0, 1, sync_attribute->time_slot[CAPCOM_SLOT],
                  &beacon_interval, &sub_cap_slot, &sleep_slot);
    slot_node_cfg(&sleep_slot, sleep_duration, 1, sync_attribute->time_slot[SLEEP_SLOT],
                  &beacon_interval, NULL, NULL);
    slot_node_cfg(&beacon_interval, 0, 0, sync_attribute->time_slot[BEACON_INTERVAL],
    ...
    {
        slot_node_cfg(&sleep_slot, 0, 1, sync_attribute->time_slot[SLEEP_SLOT], &beacon_interval, NULL, NULL);
...
static bool_t mac_frame_head_parse(pbuf_t *pbuf)
{
    ...
    DBG_LOG(DBG_LEVEL_ORIGIN, "seq:0x%x\r\n", pbuf->attri.seq);
...
void mac_beacon_frame(pbuf_t *pbuf, sync_info_t *info)
{
    ...
    info->sync_attribute->local_beacon_map |= ((uint16_t)0x01 << (bcn_pld->index));
...
#endif
#if DEBUG_COORD_ID != 0
    if(mac_short_addr_get(DEBUG_COORD_FATHER_ID) != pbuf->attri.src_id)
    {
        m_tran_recv();
        return;
    }
