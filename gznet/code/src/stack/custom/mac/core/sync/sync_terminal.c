...
//	supf_spec_t *supf = &info.sync_attribute->supf_cfg_arg;
	if(self_coord_beacon_recv_enable(slot,info.sync_attribute->supf_cfg_arg.intra_gts_number,
...
//	supf_spec_t *supf = &info.sync_attribute->supf_cfg_arg;
	...
	if(coord_intra_gts_enable(slot,info.sync_attribute->supf_cfg_arg.intra_gts_number,
                              info.sync_attribute->coord_cluster_index))
	{
...
static void super_frame_slot_cb(void *seq_p)
{
	...
	{
		uint16_t start,end;
		intra_gts_range(info.sync_attribute,&start,&end);
