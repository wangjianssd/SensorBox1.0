...
void mac_beacon_package(sbuf_t *sbuf, sync_attribute_t *sync_attribute, mac_pib_t *mac_pib)
{
    ...
    *(beacon_packet->data_p) = mac_pib->mac_seq_num++;
	...
	uint64_t id = mac_pib->mac_addr;
    uint16_t src_addr = mac_short_addr_get( id);
    osel_memcpy(beacon_packet->data_p, &src_addr, MAC_ADDR_SHORT_SIZE);
    beacon_packet->data_p += MAC_ADDR_SHORT_SIZE;
	
	sync_attribute->supf_cfg_arg.intra_channel = sync_attribute->intra_channel;
    supf_spec_t supf_config =  sync_attribute->supf_cfg_arg;
    uint16_t count = 0,short_addr_num = 0, long_addr_num= 0;
    if (!list_empty(&down_list))
    {
        list_count(&down_list,count);
        sbuf_t *pos1 = NULL;
        list_entry_for_each( pos1, &down_list, sbuf_t, list)
        {
    ...
    if(count != 0)
    {
        ...
        while(short_addr_num--)
        {
            ...
            if(!list_empty(&down_list))
            {
                ...
                list_entry_for_each( pos1 , &down_list, sbuf_t, list)
                {
                    ...
                    {
                        pbuf_free(&(pos1->primargs.pbuf)__PLINE2);
        ...
        while(long_addr_num--)
        {
            if(!list_empty(&down_list))
            {
                ...
                {
                    mac_prim_arg = &(pos1->primargs.prim_arg.mac_prim_arg);
                    if (mac_prim_arg->dst_mode == MAC_MHR_ADDR_MODE_LONG)
                    {
    ...
    beacon_payload.index = sync_attribute->self_cluster_index;
