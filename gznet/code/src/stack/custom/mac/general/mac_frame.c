...
bool_t assoc_response_frame(pbuf_t *pbuf, mac_pib_t *mac_pib,sync_attribute_t *sync_attribute)
{
    ...
    sync_attribute->self_cluster_index = res_arg.index;
...
void south_sbuf_fill(sbuf_t *sbuf, uint16_t saddr)
{
    ...
    uint64_t dst_addr = 0;
    mac_offset += get_addr(pbuf, (mac_addr_mode_e)mac_head.ctrl.des_addr_mode, &dst_addr);
    mac_head.addr.dst_addr = dst_addr;
	uint64_t addr = 0;
	osel_memcpy(pbuf->data_p, &saddr, sizeof(uint16_t));
    mac_offset += get_addr(pbuf, (mac_addr_mode_e)mac_head.ctrl.src_addr_mode, &addr);
