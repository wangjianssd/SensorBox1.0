...
static bool_t sync_check(void)
{
	if(!m_sync.find_sys_coord())
	{
		m_sync.stop();
		return FALSE;
	}
...
static bool_t asyn_check(void)
{
	if(!asyn.find_sys_coord())
	{
		asyn.stop();
		return FALSE;
	}
...
static void mac_switch(void)     /**< 启动网络切换 */
{
	...
	if(info.drivce_type != NODE_TYPE_GATEWAY && info.current_mode != RANDOM_S
	   && get_state() != WORK_ON)
	{
		bool_t result = FALSE;
		...
		if(result == TRUE || ch_index == CH_NUM)
		{
			ch_index = 0;
			return;
		}		
	...
	if(info.ch[ch_index] == 0xff)
	{
		ch_index++;
		mac_switch_timer_start(100);
		return;
	}
	
	if(info.mode == RANDOM_S && (get_state() == READY_IDLE || get_state() == WORK_DOWN))
	{
		if(info.current_mode == RANDOM_S)
		{
			info.current_mode = SYNC_S;
			info.agreement.sync_attribute.intra_channel = info.ch[ch_index];
			m_sync.run();
		...
		else if(info.current_mode == SYNC_S)
		{
			...
			info.agreement.asyn_attribute.intra_channel = info.ch[ch_index];
			asyn.run();
			mac_switch_timer_start(3000);
		}
		else if(info.current_mode == ASYN_S)
		{
			ch_index++;
			info.current_mode = RANDOM_S;
...
static void ready_idle(void)
{
	...
	mac_package.mac_pib = &info.mac_pib;
	mac_package.sync_attribute = &info.agreement.sync_attribute;
	...
	asyn_info->mac_pib = &info.mac_pib;
	asyn_info->attribute = &info.agreement.asyn_attribute;
	//同步初始化
	sync_info_t *sync_info;
	sync_info = m_sync.get();
	sync_info->drivce_type = &info.drivce_type;
	sync_info->state = &info.state;
	sync_info->mac_pib = &info.mac_pib;
	sync_info->sync_attribute = &info.agreement.sync_attribute;
...
static bool_t cb_register(void)
{
	if(mac_cb.recv_cb==NULL || mac_cb.send_cb==NULL || mac_cb.mac_assoc_cb==NULL)
		return FALSE;
	asyn_cb.recv_cb = sync_cb.recv_cb = mac_cb.recv_cb;
	asyn_cb.send_cb = sync_cb.send_cb = mac_cb.send_cb;
	asyn_cb.mac_assoc_cb = sync_cb.mac_assoc_cb = mac_cb.mac_assoc_cb;
	asyn_cb.mac_restart = sync_cb.mac_restart = mac_restart;
	return TRUE;
}
...
PROCESS_THREAD(mac_event_process, ev, data)
{
	...
	{   
		...
		else if(ev == ASYN_EVENT)
		{
			osel_pthread_create(mac_task_tcb, &asyn_process, NULL);
		}
		else
		{
...
{	
    hal_timer_init();
	//需要临时配置
	asyn_cfg_t *asyn_cfg = &info.agreement.asyn_attribute.asyn_cfg_arg;
	asyn_cfg->duration = 30;
	asyn_cfg->auery_duration = 10;
	asyn_cfg->auery_order = 10;
	asyn_cfg->asyn_cycle = asyn_cfg->duration * asyn_cfg->auery_order;
	info.mode = SYNC_S;
...
bool_t mac_run(void)
{
	...
	info.mac_pib.self_saddr = mac_short_addr_get(info.mac_pib.mac_addr);
	if(gznet_run == TRUE)
	{
