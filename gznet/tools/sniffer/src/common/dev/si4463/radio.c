...
//        radio_4259_idle();
    ...
    case RF_RX_STATE:
        ...
        si446x_set_property(0x12, 0x02, 0x11, 0x00, 0x3f);
...
//    DBG_LOG(DBG_LEVEL_INFO, "radio is 0x%x\r\n", Si446xCmd.PART_INFO.PART);
    ...
    rf_set_value(RF_STATE, RF_SLEEP_STATE);
    /*Read ITs, clear pending ones*/
    si446x_get_int_status(0u, 0u, 0u);
...
rf_result_t rf_get_value(rf_cmd_t cmd, void *value)
{
    ...
    case RF_RXFIFO_CNT:
        si446x_fifo_info(0);
        *(uint8_t *)value = Si446xCmd.FIFO_INFO.RX_FIFO_COUNT;
        return RF_RESULT_OK;
...
rf_result_t rf_set_value(rf_cmd_t cmd, uint8_t value)
{
    ...
    case RF_LADDR0:
        si446x_set_property(0x30, 0x0C, 0x00,   //*< property, addr, len
                            value, 0xFF, 0x41);   //*< match1 val, mask, ctl
        break;

    case RF_LADDR1:
        si446x_set_property(0x30, 0x0C, 0x03,   //*< property, addr, len
                            value, 0xFF, 0x02);   //*< match1 val, mask, ctl
        break;
