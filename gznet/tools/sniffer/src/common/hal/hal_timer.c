...
static void hal_timer_compare_interrupt_handler(_TIMER_CMP_LINE);
//static void hal_timer_overflow_interrupt_handler(_TIMER_OF_LINE);
...
#else // for cc2530
    // 2530平台中溢出中断的比较值可能不是0，所以一直等到溢出中断触发
...
/* 设置一个很久以后触发的时间，保证不会被触发; */
        hal_time_t stime;
#if (((__TID__ >> 8) & 0x7F) == 0x2b)     /* 0x2b = 43 dec for msp430 */
