...
static void beacon_slot_cb(void *seq_p)	//beacon时隙
{
	...
	if(self_coord_beacon_recv_enable(slot,supf->intra_gts_number,info.mac_pib->coord_cluster_index))
	{//1.接收父节点的beacon
		m_tran_recv();
	...
	else
	{
		...
		else if(beacon_recv_enable(slot,supf->intra_gts_number,info.mac_pib->local_beacon_map))
		{//侦听邻居beacon
			m_tran_recv();
...
static void intra_gts_slot_cb(void *seq_p)	//簇内时隙
{
	...
	if(coord_intra_gts_enable(slot,supf->intra_gts_number,info.mac_pib->coord_cluster_index))
	{//在上级设备的簇内时隙
...
static void intracom_slot_cb(void *seq_p)
{
	...
	{
		uint16_t start,end;
		inter_gts_range(info.mac_pib,&start,&end);
...
static void inter_gts_slot_cb(void *seq_p)	//簇间时隙
{
	uint16_t slot = m_slot_get_seq();
	supf_spec_t *supf = &info.mac_pib->supf_cfg_arg;
	if(self_coord_inter_gts_enable(slot,info.mac_pib))		//1.是上级的簇间时隙发送数据
	{
	...
	else if(self_inter_gts_enable(slot,info.mac_pib))	//2.是自己的簇间时隙接收数据
	{
		m_tran_recv();
		mac_sched_recv_timeout_set(FALSE);
	}
...
static void mac_beacon(pbuf_t *pbuf)
{
	...
	{
		uint8_t cluster_index = m_slot_get_seq()/info.mac_pib->supf_cfg_arg.cluster_number+1;
