...
//    while(1)
//    {
//        radio_SpiWriteByte(0xCC);    //test SPI write
//        radio_SpiWriteByte(0xAA);    //test SPI write
//        radio_SpiWriteByte(0xE7);    //test SPI write
//    }
    
    radio_SpiWriteByte(0x44);    //read CMD buffer
    ctsVal = radio_SpiReadByte(0xFF);
    if (ctsVal == 0xFF)
    {
      if (byteCount)
      {
        radio_SpiReadData(byteCount, pData);
      }
      radio_set_nsel();
      break;
    }
...
void radio_comm_SendCmd(uint8_t byteCount, uint8_t* pData)
{
    ...
    radio_clear_nsel();
    radio_SpiWriteData(byteCount, pData);
    radio_set_nsel();
    ctsWentHigh = 0;
}
