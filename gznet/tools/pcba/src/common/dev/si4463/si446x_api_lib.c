...
#include <inc/radio.h>
#include <inc/radio_comm.h>
     
#include <inc/si446x_api_lib.h>
#include <inc/si446x_cmd.h>
...
void si446x_reset(void)
{
    ...
    radio_AssertShutdown();
    //! @todo this needs to be a better delay function.
    for (loopCount = 4096; loopCount != 0; loopCount--);
    radio_DeassertShutdown();
...
U8 si446x_configuration_init(const U8* pSetPropCmd)
{
  ...
  /* While cycle as far as the pointer points to a command */
	uint8_t pData = 0;/* Walker changed */
  while (*pSetPropCmd != 0x00)
  {
    ...
    if (radio_comm_SendCmdGetResp(numOfBytes, Pro2Cmd, 0, &pData) != 0xFF)
    {
      /* Timeout occured */
      return SI446X_CTS_TIMEOUT;
    }

    if (radio_get_NirqLevel() == 0)
    {
...
{
  radio_comm_ReadData( SI446X_CMD_ID_READ_RX_FIFO, 1, numBytes, pRxData );
}
