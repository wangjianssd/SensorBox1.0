                        ...
                        coap_content_type_t content_type)
{
    ...
    pkt->numopts = 1;
#if 0
    scratch->p[0] = ((uint16_t)content_type & 0xFF00) >> 8;     // 到底是一个字节还是两个字节，RFC文档没有说明
    ...
    pkt->opts[0].buf.p = scratch->p;                            // 临时区域scratch已经装载Option，直接复制给第一个Option
    if (scratch->len < 2)
        return COAP_ERR_BUFFER_TOO_SMALL;
    pkt->opts[0].buf.len = 1;									// content_type 修改为一个字节
...
next:
    ...
    // 匹配不成功，返回404
    coap_make_response(scratch, outpkt, NULL, 0,
                       inpkt->hdr.id[0], inpkt->hdr.id[1],
                       &inpkt->tok,
